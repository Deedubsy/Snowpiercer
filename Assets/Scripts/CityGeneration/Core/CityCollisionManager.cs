...
namespace CityGeneration.Core
{
    ...
    public class CityCollisionManager
    {
        ...
        private SpatialGrid<Vector3Wrapper> roadNetworkGrid;
        ...
        public void Initialize(float citySize)
        {
            ...
            roadNetworkGrid = new SpatialGrid<Vector3Wrapper>(cellSize, bounds);
        ...
        {
            var wrapper = new Vector3Wrapper(position, width * 0.5f, $"road_{position.x}_{position.z}");
            roadNetworkGrid.AddObject(wrapper, position, width * 0.5f);
        }
        ...
        public Vector3 GetNearestRoadPoint(Vector3 position)
        {
            ...
            return spatialObj?.obj?.position ?? position;
        }
    ...
    // ObjectType moved to CityTypes.cs
}